...
foreach ($orders as $key => $value) {
	foreach ( $value->get_items() as $item_id => $item ) {
		...
		if (!empty($date) && !empty($time)) {
			...
			array_push($all_valid_order,array('slot' => $time,'product' => $item->get_name(),'quantity' => $item->get_quantity(),'name' => $value->get_billing_first_name().' '.$value->get_billing_last_name(),'image_url' =>  $image[0],'order' => $value->get_id() ));
		}
		else{
			continue;
		}
